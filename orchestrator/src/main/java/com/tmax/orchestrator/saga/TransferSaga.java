...
import com.tmax.orchestrator.framework.SagaState;
...
public class TransferSaga extends SagaBase {
    ...
    protected TransferSaga(Event<ExportedEvent<?,?>> event, SagaState state) {
        super(event, state);
    }

    @Override
    protected SagaStepMessage getStepMessage(String topic) {

        if (topic.equals(RECIPIENT_ACCOUNT_UPDATE)) {
            return new SagaStepMessage(RECIPIENT_ACCOUNT_UPDATE, REQUEST, getPayload());
        }
        else if (topic.equals(DEPOSITOR_ACCOUNT_UPDATE)) {
            return new SagaStepMessage(DEPOSITOR_ACCOUNT_UPDATE, REQUEST, getPayload());
        }
        else if (topic.equals(DEPOSIT_WITHDRAWAL_TX_INSERT)){
            return new SagaStepMessage(DEPOSIT_WITHDRAWAL_TX_INSERT, REQUEST, getPayload());
        }
        else if (topic.equals(TRANSFER_TX_INSERT)){
            return new SagaStepMessage(TRANSFER_TX_INSERT, REQUEST, getPayload());
        }
        else if (topic.equals(RECIPIENT_ACCOUNT_APPROVAL)){
            return new SagaStepMessage(RECIPIENT_ACCOUNT_APPROVAL, REQUEST, getPayload());
        }
        else if (topic.equals(DEPOSITOR_ACCOUNT_APPROVAL)){
            return new SagaStepMessage(DEPOSITOR_ACCOUNT_APPROVAL, REQUEST, getPayload());
        }
